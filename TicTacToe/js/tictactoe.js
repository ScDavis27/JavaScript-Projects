...
// Disabe the stop button since it is not needed until the game start.
...
function stopEnabled(btn) {
    btn.style.color = "#fff";
    btn.style.border = "2px solid rgb(24, 0, 0)";
...
function stopGame() {
    ...
    var btn = document.getElementById('btnStart');
    startEnabled(btn); // enabe the start button since the game is now stopped
    var btn = document.getElementById('btnStop');
...
function hideGameMsg() {
    clearMsg() // clear the text from message console
...
// this function is for the player coonfiguration panel and checks the
...
function check4Tie() {
    ...
    if(boardState.length >= 9 && check != "That's three in a row, Player 1 wins!" && check != "That's three in a row, Player 2 wins!") {
        ...
        tieSound(); // play a sound when a tie game has been detected 
        writeMsg(txt1);
...
//---------------------------------------------------------------------
// checking wincon squares 012
function checkWinCon1(info,squaresArray) {
    winDetected = "on";
    var winCon1 = [0,1,2];
    ...
    for (var i in info) {
        if (info[i].charAt(0) == "0") {
            var match0Avatar = info[i].charAt(1); // only interested in their avatar
        }
